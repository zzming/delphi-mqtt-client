...
type
  ...
  TMQTTRecvState = (FixedHeaderByte, RemainingLength, RemainingLength1,
    RemainingLength2, RemainingLength3, RemainingLength4, Data);
  ...
  TPublishEvent = procedure(Sender: TObject; QoS, MessageID: Integer;
    topic, payload: UTF8String) of object;
  ...
  TSubAckEvent = procedure(Sender: TObject; MessageID: Integer;
    GrantedQoS: Array of Integer) of object;
  ...
  TMQTTMessageIdVarHeader = class(TMQTTVariableHeader)
  private
    FMessageID: Integer;
    function get_MessageID: Integer;
    procedure set_MessageID(const Value: Integer);
  ...
  TMQTTSubscribeVarHeader = TMQTTMessageIdVarHeader;
  TMQTTUnsubscribeVarHeader = TMQTTMessageIdVarHeader;
  ...
  public
    constructor Create;
    procedure ClearField;
    procedure AddField(ABytes: TBytes; APerpendLength: boolean = True);
      overload;
    procedure AddField(AByte: Byte; APerpendLength: boolean = True); overload;
    procedure AddField(AUTF8Str: UTF8String;
      APerpendLength: boolean = True); overload;
    function ToBytes: TBytes; virtual;
...
procedure SetDWordBits(var Bits: Byte; const aIndex: Integer;
...
begin
  ...
  // Result[i + 2] := temp[i];
  // end;
  Move(temp[0], Result[2], Length(temp));
...
var
  ...
  // ==============================================================================
begin
  Result := TEncoding.UTF8.GetBytes(AStrToEncode);
  ...
  // // Result[i] := temp[i];
  // // end;
  // Move(temp[0], Result[0], Length(temp));
  // // ==============================================================================
end;

procedure AppendToByteArray(ASourceBytes: TBytes;
...
procedure AppendToByteArray(ASourceByte: Byte;
  var ATargetBytes: TBytes); overload;
var
  iUpperBnd: Integer;
begin
...
begin
  ...
  Self.FConnectFlags.CleanStart := 0;
  Self.FConnectFlags.WillQoS := 0;
  Self.FConnectFlags.WillRetain := 0;
  Self.FConnectFlags.WillFlag := 0;
  Self.FConnectFlags.UserName := 1;
  Self.FConnectFlags.Password := 1;
  Self.FKeepAlive := 600;
end;
...
{ TMQTTMessageIdVarHeader }

constructor TMQTTMessageIdVarHeader.Create(MessageID: Integer);
begin
  inherited Create;
  FMessageID := MessageID;
end;

function TMQTTMessageIdVarHeader.get_MessageID: Integer;
begin
  Result := FMessageID;
end;

procedure TMQTTMessageIdVarHeader.set_MessageID(const Value: Integer);
begin
  FMessageID := Value;
end;

function TMQTTMessageIdVarHeader.ToBytes: TBytes;
begin
  ClearField;
  AddField(TMQTTUtilities.IntToMSBLSB(FMessageID));
  Result := FBytes;
...
procedure TMQTTPayload.AddField(ABytes: TBytes; APerpendLength: boolean = True);
var
  DestUpperBnd: Integer;
  vMsbLsb: TBytes;
begin
  if APerpendLength then
  begin
    vMsbLsb := TMQTTUtilities.IntToMSBLSB(Length(ABytes));
    AppendToByteArray(vMsbLsb, FBytes);
  end;
  AppendToByteArray(ABytes, FBytes);
end;

procedure TMQTTPayload.AddField(AByte: Byte; APerpendLength: boolean = True);
var
  vBytes: TBytes;
begin
  SetLength(vBytes, 1);
  vBytes[0] := AByte;
  AddField(vBytes, APerpendLength);
end;

procedure TMQTTPayload.AddField(AUTF8Str: UTF8String;
  APerpendLength: boolean = True);
var
  vBytes: TBytes;
begin
  vBytes := TMQTTUtilities.UTF8EncodeToBytesNoLength(AUTF8Str);
  AddField(vBytes, APerpendLength);
end;

procedure TMQTTPayload.ClearField;
begin
  SetLength(FBytes, 0);
end;
